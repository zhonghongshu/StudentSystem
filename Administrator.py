...
from ClassInformation import maintainClassInfomation
from StudentInforma import maintainStudentInfomation
from TeacherInformation import maintainTeacherInfomation
...
def administratorMainWindow(root):
    root1 = Toplevel(root)
    ...
    btn2 = Button(root1, text='输入成绩', command=lambda:ManageGrade(root1,combobox.get()))
    ...
    filemenu.add_command(label="学生表", command=lambda:maintainStudentInfomation(root1))
    filemenu.add_command(label="课程表", command=lambda:maintainClassInfomation(root1))
    filemenu.add_command(label="教师表", command=lambda:maintainTeacherInfomation(root1))
    filemenu.add_separator()
    menubar.add_cascade(label="维护", menu=filemenu)
