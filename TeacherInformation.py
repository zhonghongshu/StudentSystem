...
def newdelete(treeview1,sno,root1):
    if(tkinter.messagebox.askyesno(message='删除信息不可恢复！\n确定是否删除该信息！')):
        db=connectDB()
        pre=getteacherCount(db)
        deleteteacher(db,sno)
        deletenewaccount(db,sno)
        cur=getteacherCount(db)
        closeDB(db)
        if(pre!=cur):
            tkinter.messagebox.showinfo(message='删除信息成功！')
        else:
            tkinter.messagebox.showinfo(message='删除信息失败！此教师号不存在！')
        refresh(root1,treeview1)
...
def add(root,treeview1):
    ...
    lb6.place(relx=0.025, rely=0.5, relwidth=0.4, relheight=0.1)
    inp6 = Entry(root1)
    inp6.place(relx=0.3, rely=0.5, relwidth=0.4, relheight=0.1)
    lb7 = Label(root1, text='请输入密码：')
    lb7.place(relx=0.025, rely=0.6, relwidth=0.4, relheight=0.1)
    inp7 = Entry(root1)
    inp7.place(relx=0.3, rely=0.6, relwidth=0.4, relheight=0.1)

    
    btn1 = Button(root1, text='添加信息',command=lambda:newadd(treeview1,inp1.get(),inp2.get(),inp3.get(),inp4.get(),inp6.get(),inp7.get(),root))
...
def delete(root,treeview1):
    ...
    lb1 = Label(root1, text='请输入要删除的教师号：')
...
def maintainTeacherInfomation(root):
    root1 = Toplevel(root)
    ...
    closeDB(db)
    columns1 = ("教师号", "教师名","所在系","类别","登录名","密码")
    treeview1 = ttk.Treeview(root1, height=100, show="headings", columns=columns1)
    treeview1.column("教师号", width=30, anchor='center') 
    treeview1.column("教师名", width=30, anchor='center') 
    treeview1.column("所在系", width=30, anchor='center')
